...
%
    ...
    switch(lower(type))
        case 'add-dist'
            ...
            dist = varargin{3};
            validateattributes(dist, {'srt.disturbances.RandomVector', 'Polyhedron'}, ...
                {'nonempty'}, 'getDubinsCarLtv', ...
                'dist with add-dist option');
            if length(varargin) > 3
                throwAsCaller(SrtInvalidArgsError('Too many input arguments'));
            end
            sys = srt.systems.LtvSystem('A', @(t) eye(2), ...
                'B', time_varying_matrix, ...
                'InputSpace', velocity_input, ...
                'w', dist, ...
                'F', dist_matrix);
        case 'vel-dist'
            if isempty(varargin)
            ...
            sys = LtvSystem('A', @(t) eye(2), ...
                'F', time_varying_matrix, ...
                'w', velocity_dist);
        otherwise
    end
end
